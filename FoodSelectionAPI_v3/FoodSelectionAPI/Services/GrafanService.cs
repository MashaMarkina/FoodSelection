...
public class GrafanService : IFoodProductService
{
    private readonly IMongoCollection<FoodProduct> _foodProducts;
    private readonly FoodMetrics _foodMetrics;
    MongoDbContext mongoDB;

    public GrafanService(IOptions<MongoDbSettings> contextDB, FoodMetrics foodMetrics)
    {
    ...
    public async Task<FoodProductResponseDto> CreateAsync(FoodProductCreateDto createDto)
    {
        ...
        _foodMetrics.ProductCreated(product.Category);
        _foodMetrics.RecordDbOperationDuration(stopwatch.ElapsedMilliseconds, "insert_one");
